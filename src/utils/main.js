...
class HeaderManager {
  ...
  initMobileMenu() {
    ...
    if (hamburger && mobileNav && closeBtn && overlay) {
      // Check if we want drawer variant (you can add a data attribute to control this)
      const useDrawer = mobileNav.classList.contains('mobile-nav--drawer');
      
      // Hamburger menu click event
      hamburger.addEventListener('click', () => {
        this.toggleMobileMenu(true, useDrawer);
      });
      
      // Close button click event
      closeBtn.addEventListener('click', () => {
        this.toggleMobileMenu(false, useDrawer);
      });
  ...
  toggleMobileMenu(open, isDrawer = false) {
    const mobileNav = document.getElementById('mobile-nav');
    const hamburger = document.querySelector('.header__hamburger');
